  ...
  function UpdateItem()  { 
      ...
      if(selectedCount==0) {
            ...
            return;		        
             ...
             window.location.href= 'editView.action?viewVo.id='+thisfrm.elements[selectedOrder].value;
  ...
  function DeleteItem() {
      ...
      if(selectedCount==0) {
            ...
            return;		        
             ...
             window.location.href= 'deleteView.action?viewVo.id='+thisfrm.elements[selectedOrder].value;
  	 ...
  	 function delopt() {
  		     ...
  		     for(var i=0;i<window.opts.length;i++) {
  		        ...
  		        if(count==0) {
  		         ...
  		         // alert(frames["rightPanel"].opts[checkedopt].value);
  		         window.location.href= 'deleteView.action?viewVo.id='+window.opts[checkedopt].value;;
...
<BODY>
 <div class="ToolBar">
        <span class="ToolBarButton">
            	...
            	<a class="Btn_Add" id="Btn_Add" onmouseover='javaScript:this.className="Btn_Add_Over"' onmouseout=javaScript:this.className="Btn_Add" href="addView.action"></a>
...
<form id="ProductTable" name="ProductTable" method="post" action="viewrecords.action" >
    <div class="SheetBody">
        
		<table class="GridBorder" cellspacing="1px"  width="60%">
            <tr>       	
                <td class="GridHeader" width="10%"><input id="Id_chkAll" type="checkbox" name="Id_chkAll" onclick="javascript:return SelectAll(this.checked,this.id);" /></td>
                <td class="GridHeader" width="10%">序号</td>                
                <td class="GridHeader" width="30%">视图名称</td> 
                <td class="GridHeader" width="30%">视图标识</td>
                <td class="GridHeader" width="20%">修改日期</td>            
   
            </tr>
            <c:set var="pageData" value="${viewVo.page}"/> 
              ...
              <c:choose>
                <c:when test="${order%2!=0}"> 
                    <tr class="GridBodyA" onmouseover="this.style.backgroundColor='#E8F2FC'" onmouseout="this.style.backgroundColor=''">				
					     ...
					     <td>${order}</td>			    		 
						 <td style="text-align:left;padding-left:8px;" ><a href="showView.action?viewVo.id=${entity.id}" style="a{color:#00F;}a.link{ color:#00F}a.hover{color:#F00}" >${entity.name}</a></td>
						 <td>${entity.viewId}</td>
	...
	<div class='Pager'>
	<input type="hidden"  name="cur" id="cur"  value="${pageData.currentPage}"  />
       <table>
         <tr>
           <td>
