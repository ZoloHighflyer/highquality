   ...
   function updateopt() {
  	...
  	function requestPage(curpage) {
		...
		document.getElementById("currentPage").value = curpage;
	   	window.location.href= 'viewgroups.action?pagi.currentPage=' + curpage;
    } 
  </script>
</HEAD>
...
<BODY>
<!-- 
<a href="addgroup.action">增加组</a>&nbsp;&nbsp;&nbsp;&nbsp;
<a href="javascript:updateopt();">修改组</a>&nbsp;&nbsp;&nbsp;&nbsp;
<a href="javascript:delopt();">删除组</a>
 -->

	<div class="ToolBar">
	    <span class="ToolBarButton">
	        <a class="Btn_Add" id="Btn_Add" onmouseover='javaScript:this.className="Btn_Add_Over"' onmouseout='javaScript:this.className="Btn_Add"' href="addgroup.action"></a>
	...
	<div class="SheetBody" style="width:60%">
		<table class="GridBorder" cellspacing="1px" border="0" style="width:100%;">
			<tr>
			   <td class="GridHeader" width="35">选择</td>
			   <td class="GridHeader" width="35">序号</td>
			   <td class="GridHeader">组名称</td>
			</tr>
				...
				<c:choose>
					<c:when test="${order%2 != 0}">
						<tr class="GridBodyA" onmouseover="this.style.backgroundColor='#E8F2FC'" onmouseout="this.style.backgroundColor=''">
						  <td> 
						  	<input id="c${order}" name="opts" type="checkbox" value="${groupVo.id}"/>
						  </td>
						  <td>${order}</td>
						  <td>${groupVo.name}</td> 
						</tr>
