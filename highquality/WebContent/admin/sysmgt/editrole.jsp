...
<HTML>
 <HEAD> 
  ...
  <link href="<%=ROOTPATH%>/jscomponents/erp.css" type="text/css" rel="stylesheet" />  <script src="<%=ROOTPATH%>/auth/dtree/dtree.js"></script>  
...
<body topmargin="0">
	...
	<form method="POST" action="saverole.action">
		<input type="hidden" name="roleVo.action" value="edit_action"/>
		...
		<div class="EditDivBody">
		  <table cellspacing="1" border="0" class="EditTableBorder"  width="40%" >
			...
			<tr>
		        ...
		        <td class="EditRow">
		           <input type="text" class="text" name="roleVo.name" value="${roleVo.name}" size="55"/>
		    	</td>
			</tr>
			<tr>
				<td class="EditHead">描述：</td>
		        <td class="EditRow">  	
		            <input type="text" class="text" name="roleVo.descr" value="${roleVo.descr}" size="55"/>
		        </td>
			</tr>
			  ...
			  <c:forEach items="${roleVo.node.children}" var="roleleveltwo" >       
			    <tr>
			    	<td colspan="2" style="background-color: white">	    
		          		<input id="2#${rolelevelone.nodeid}" name="roleVo.roleNodes" type="checkbox" value="2#${roleleveltwo.nodeid}"
		             		<c:choose>
			            		<c:when test="${roleleveltwo.selected}">
				        			checked
				        		</c:when>
				     		</c:choose> 
		          		/>${roleleveltwo.nodename}<br>
				        <c:forEach items="${roleleveltwo.children}" var="rolelevelthree" >		          
				           &nbsp;&nbsp;&nbsp;&nbsp;<input id="3#${rolelevelthree.nodeid}" name="roleVo.roleNodes" type="checkbox" value="3#${rolelevelthree.nodeid}"  
				              <c:choose>
			            	    <c:when test="${rolelevelthree.selected}">
				                 checked
				                </c:when>
				              </c:choose>
				           />${rolelevelthree.nodename}<br>
				             
				             <c:forEach items="${rolelevelthree.children}" var="rolelevelfour" >
				               &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;	
				               <input id="4#${rolelevelfour.nodeid}" name="roleVo.roleNodes" type="checkbox" value="4#${rolelevelfour.nodeid}"  
				                   <c:choose>
			            	          <c:when test="${rolelevelfour.selected}">
				                      checked
				                      </c:when>
				                   </c:choose>
				               />${rolelevelfour.nodename}<br>
				             </c:forEach>
