...
ValidationFramework.validateDepend = function(field, depend) {
	...
	} else if (depend.getName() == "float") {
		return ValidateMethodFactory.validateFloat(field, depend.getParams());
	} else {
...
ValidateMethodFactory.validateFloat = function(field, params) {
	var obj = ValidationFramework.getWebFormFieldObj(field);
	if (obj == null) return false;
	if (obj.value == "") return true;
	var exp = new RegExp("^\\d+(\\.\\d+)?$");
...
ValidationErrorString["zh-cn"]["float"]="{0}必须是正浮点数。";
////English(en-us)
...
ValidationErrorString["en-us"]["float"]="{0} should be postive float. ";
