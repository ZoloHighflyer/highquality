...
public class RoleService implements IRoleService {
	...
	public IServiceResult useServiceByAction(BaseVo bsvo) throws ServiceException {
		...
		if (IRoleService.ACTION_CREATE_ROLE.equals(bsvo.getAction())) {
    	...
    	try {
			...
			if ((roles!=null)&&(roles.size()>0)) {
				for(int i=0;i<roles.size();i++) {
					...
					rolevos.add(roleToRoleVo(role));
				}				
			}			
		} catch (DaoException e) {			
			e.printStackTrace();
			log.error(this.getClass().getName()+" findAllRoleVos() fail! "+e.getMessage());
			throw new ServiceException(this.getClass().getName()+" findAllRoleVos() fail!");
		}
        return rolevos;
    }
    ...
    public List<RoleVo> findAllRoleVos(Pagination pagi) throws ServiceException {
    	...
    	try {
			// List roles = roleDao.findAllEntities();
    		Pagination pg = roleDao.findEntitiesByPage(pagi);
    		List roles = pg.getList();
    ...
    public RoleVo findRoleById(long roleid) throws ServiceException{
    	...
    	try {
			...
			if (role==null) {
				log.error(this.getClass().getName()+" findRoleById(long roleid) fail! can not find role by id: "+role.getId());
				throw new ServiceException(this.getClass().getName()+" findRoleById(long roleid) fail! can not find role by id: "+role.getId());
			}
	...
	public IServiceResult createRole(BaseVo bsvo) throws ServiceException{
		...
		try {
			Set<RoleNode> roleNodes = role.getRoleNodes();
			for (RoleNode roleNode : roleNodes) {
				long l = roleNode.getParentNo();
				String sl = String.valueOf(l);
				if ("-1".equals(sl)) {
					roleNode.setParentNo(0L);
				}
			}
